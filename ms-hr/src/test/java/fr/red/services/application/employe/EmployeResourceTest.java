package fr.red.services.application.employe;

import com.fasterxml.jackson.core.JsonProcessingException;
import com.fasterxml.jackson.core.type.TypeReference;
import com.fasterxml.jackson.databind.ObjectMapper;
import fr.red.services.application.employe.model.EmployeUI;
import fr.red.services.application.employe.rest.EmployeResource;
...
public class EmployeResourceTest {
    ...
    public void testGetEmployes() throws Exception {
        ...
        MvcResult mvcResult = mockMvc.perform(MockMvcRequestBuilders.get("/private/api/v1/employes").contentType(MediaType.APPLICATION_JSON)).andExpect(
    ...
    public void testGetEmployeById() throws Exception {
        ...
        MvcResult mvcResult = mockMvc.perform(
                MockMvcRequestBuilders.get("/private/api/v1/employes/{id}", "7369").contentType(MediaType.APPLICATION_JSON)).andExpect(
                MockMvcResultMatchers.status().isOk())
                                     .andReturn();
        int status = mvcResult.getResponse().getStatus();
        Assert.assertEquals(200, status);
    ...
    public void testAddEmploye() throws Exception {
        ...
        MvcResult mvcResult = mockMvc.perform(MockMvcRequestBuilders.post("/private/api/v1/employes").contentType(MediaType.APPLICATION_JSON).content(inputJson)).andReturn();
        int status = mvcResult.getResponse().getStatus();
    ...
    public void testUpdateEmploye() throws Exception {
        ...
        Mockito.when(employeRequester.updateEmploye(ArgumentMatchers.any())).thenReturn(getExpectedEmploye());
        EmployeUI employeUIToUpdate = EmployeUI.builder()
                                            .empno("7369")
                                            .ename("SMITH")
                                            .job("CLERK")
                                            .hiredate(toDate("17-12-1980"))
        ...
        String inputJson = mapToJson(employeUIToUpdate);
        MvcResult mvcResult = mockMvc.perform(MockMvcRequestBuilders.put("/private/api/v1/employes").contentType(MediaType.APPLICATION_JSON).content(inputJson)).andReturn();
        int status = mvcResult.getResponse().getStatus();
        Assert.assertEquals(200, status);
        ObjectMapper mapper = new ObjectMapper();
        EmployeUI result = mapper.readValue(mvcResult.getResponse().getContentAsByteArray(),
    ...
    public void testDeleteEmploye() throws Exception {
        Mockito.when(employeRequester.getEmployeById("7369")).thenReturn(getExpectedEmploye());
        MvcResult mvcResult = mockMvc.perform(
                MockMvcRequestBuilders.delete("/private/api/v1/employes/{id}", "7369").contentType(MediaType.APPLICATION_JSON)).andReturn();
        int status = mvcResult.getResponse().getStatus();
        Assert.assertEquals(200, status);
        String content = mvcResult.getResponse().getContentAsString();
        ObjectMapper mapper = new ObjectMapper();
