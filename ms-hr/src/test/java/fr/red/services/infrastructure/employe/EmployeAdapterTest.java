package fr.red.services.infrastructure.employe;

import fr.red.services.domain.department.model.Department;
import fr.red.services.domain.employe.model.Employe;
import fr.red.services.infrastructure.employe.referentiel.adapter.EmployeAdapter;
...
public class EmployeAdapterTest {
    ...
    public void testAddEmploye() throws ParseException {
        ...
        Mockito.when(employeRepository.saveOrUpdate(refEmployeToAdd)).thenReturn(expectedRefEmploye);
    ...
    public void testUpdateEmploye() throws ParseException {
        ...
        RefEmploye refEmployeToUpdate = RefEmploye.builder()
                                               .empno("7369")
                                               .ename("SMITH")
                                               .job("CLERK")
                                               .hiredate(toDate("17-12-1980"))
        ...
        Mockito.when(employeRepository.saveOrUpdate(refEmployeToUpdate)).thenReturn(getExpectedUpdatedRefEmploye());

        Employe result = employeAdapter.updateEmploye(populateEmploye());

        Assert.assertNotNull(refEmployeToUpdate);
        Assert.assertEquals("7369", result.getEmpno());
        Assert.assertEquals("SMITH", result.getEname());
        Assert.assertEquals("CLERK", result.getJob());
        Assert.assertEquals(toDate("17-12-1980"), result.getHiredate());
        Assert.assertEquals("updated salary", Double.valueOf(1800d), result.getSalary());
        Assert.assertEquals("moving to new department","10", result.getDepartment().getDeptno());
        Assert.assertNull(result.getDepartment().getDname());
        Assert.assertNull(result.getDepartment().getLoc());
    }
    ...
    private RefEmploye getExpectedUpdatedRefEmploye() throws ParseException {
        return RefEmploye.builder()
                         .empno("7369")
                         .ename("SMITH")
                         .job("CLERK")
